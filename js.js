...
const map = {
    ...
    score(currentSnakeBody){
        const score = document.getElementById('score');
        score.innerHTML = `Счет: ${currentSnakeBody.length-1}`;
    },

    render(snakePointsArray, foodPoint, barrierArray) {
        ...
        barrierArray.forEach((point, index) => {
            const barCellKey = `x${point.x}_y${point.y}`;
            const barrierCell = this.cells[barCellKey];
...
const barrier = {
    ...
    init(barrierCurrentArrow) {
        this.barrierSet = barrierCurrentArrow;
    },
    ...
    isOnPoint(point) {
        return this.getbarrierSet().some(snakePoint => {
...
const game = {
    ...
    reset() {
        ...
        this.barrier.init(this.getBarrierArrow());
        this.render();
    },

    render() {
        this.map.render(this.snake.body, this.food.getCoordinates(), this.barrier.barrierSet);
        this.map.score(this.snake.body);
    },
    ...
    getBarrierArrow(){
        ...
        for (let j = 0; j < 3; j++) {
        result.push(this.getRandomFreeCoordinates())
        };
    ...
    getRandomFreeCoordinates() {
        const exclude = [this.food.getCoordinates(), ...this.barrier.getbarrierSet(), ...this.snake.getBody()];
    ...
    tickHandler() {
        ...
        if (this.barrier.isOnPoint(this.snake.getNextHeadPoint())) {
            document.getElementById("score").innerHTML= "Упс, змея расшиблась о препятствие!";
            return this.finish();
        }
